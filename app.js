...
function applyStaticText(){
  els.heroEyebrow.textContent=text(setting("Hero Eyebrow","BAR MENU"));

  els.heroTitle.innerHTML=text(
    setting("Hero Title","Curated pours.<br>Made for the night.")
  );

  els.heroSubtitle.textContent=text(
    setting("Hero Subtitle","The current selection at GYLT.")
  );

  els.eventSectionLabel.textContent=text(
    setting("Event Section Label","TONIGHT AT GYLT")
  );

  els.sponsorKicker.textContent=text(
    setting("Sponsor Kicker","THIS WEEK'S")
  );

  els.sponsorLine.textContent=text(
    setting("Sponsor Line","EXCLUSIVE POURS")
  );

  els.menuLabel.textContent=text(
    setting("Menu Label","BAR MENU")
  );

  els.menuTitle.textContent=text(
    setting("Menu Title","Tonight's Selection")
  );

  els.searchInput.placeholder=text(
    setting("Search Placeholder","Search a pour")
  );

  els.clearSearch.textContent=text(
    setting("Search Clear Text","Clear")
  );

  els.searchToggle.setAttribute(
    "aria-label",
    text(setting("Search Button Label","Search menu"))
  );

  els.loadingText.textContent=text(
    setting("Loading Text","Loading tonight's menu…")
  );

  els.emptyTitle.textContent=text(
    setting("Empty State Title","No match found")
  );

  els.emptyCopy.textContent=text(
    setting("Empty State Copy","Try another brand, category or product.")
  );

  els.footerLocation.textContent=text(
    setting("Footer Location","BENGALURU")
  );

  els.footerNote.textContent=text(
    setting(
      "Footer Note",
...
function applySettings(settings){
  ...
  if(reserveUrl){
    ...
    els.reserveButton.textContent=text(
      setting("Reserve Button Text","RESERVE A TABLE")
    );
    ...
    (group,groupIndex)=>{
      ...
      group.items.forEach(item=>{
        ...
        if(
          ...
          meta.textContent=
            item.soldOut
              ? text(setting("Sold Out Text","Currently unavailable"))
              : metaParts.join(" · ");
        ...
        price.textContent=
          item.soldOut
            ? text(setting("Sold Out Price Text","—"))
            : money(item.price);
        ...
        if(item.featured){
          ...
          badge.textContent=
            text(setting("Featured Badge Text","GYLT SELECT"));
